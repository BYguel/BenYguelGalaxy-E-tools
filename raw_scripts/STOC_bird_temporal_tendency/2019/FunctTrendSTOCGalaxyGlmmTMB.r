...
suppressMessages(library(lme4))##  version 1.1.18.1    required for the confidence interval calculation of the regression coeffficeint of the mixed effects models
...
suppressMessages(library(sjstats))  ## version 0.17.4   required to calculate the R squared of mixed effect models
suppressMessages(library(dplyr))  ##  version 0.8.0.1  required to use the sjstats package and the function r2
suppressMessages(library(glmmTMB))   ## Version: 0.2.3  required to use the mixed effect model with more options than lmer or lme4  
...
############################################################################################ debut de la Function main.glm / start of the function main.glm   ########## ATTENTION FONCTION glmmTMB NE MARCHE PAS AVEC CERTAINES DONNEES AVEC FORTE VALEURE ex APUAPU (Warning message:In sqrt(diag(vcov)) : NaNs produced)
...
################################################################# SPECIFICATION MODEL DEBUT NOUVEL VERSION LMER
###############################  
...
# browser()
   ### Utilisation des modèles mixtes pour obtenir les tendances d evolution par an du csi cti ou ctri / Use of mixte model for the estimation of the annual variations of the csi cti or ctri 
		...
		AjustR2fix <- dispAn$rsq.marginal
		AjustR2fixrand <-dispAn$rsq.conditional 
...
{	
        ...
        ## indice de surdispersion  / overdispersion index REPLACED BY MEASURE OF R SQUARED
       ## browser()
        #if(assessIC) dispAn <- glm1$deviance/glm1$null.deviance else dispAn <- glm1$deviance/glm1$nulldev ##### MARCHE PAS AVEC glmmTMB   
    ...
    cat("\nEstimation de la tendance sur ",nbans," ans avec glmmTMB(",sp,"~ annee +(1|carre))\n",sep="")
       ...
       smd2 <- coefficients(smd2)$cond ### coefficient regression de chaque variable avec les résultats des tests statistiques / regression coefficient of each predictive variables with results of the statistical tests
       smd2 <- tail(smd2,1)       ### coefficient regression de variable annee avec les résultats des tests statistiques / regression coefficient of the variable year with results of the statistical tests
        ...
        UL <- NA
        if(assessIC) {
				MODconfint2 <- confint(md2) #### plus rapide de passer par la fonction profile mais pas indispensable fonctionne aussi directement sur modele mixte md.f  / more rapid using both function profile and confint but works also directly on output of the model 
		se.sup <- MODconfint2[2,2]#### [2:nban+2,2] version pour sortie lmer()
		UL <- exp(se.sup)
        ...
        ## mesure de la surdispersion / overdispersion measurment
						...
						AjustR2fixrand2 <- dispTrend$rsq.conditional
       ...
       # browser()
        ...
        if(AjustR2fixrand > AjustR2fix | AjustR2fixrand2 > AjustR2fix2 | AjustR2fix < 0.20 | AjustR2fix2 < 0.20 | median( nb_carre_presence)<seuilOccu) {
            ...
            if(AjustR2fixrand > AjustR2fix | AjustR2fixrand2 > AjustR2fix2 | AjustR2fix < 0.20 | AjustR2fix2 < 0.20 ) {
                vecLib <- c(vecLib,"low R2")
            }
        }
     ...
     #   browser()
        tabTrend <- data.frame(
            ...
            valide = catIncert,raison_incertitude = raisonIncert,AjustR2fix=AjustR2fix2,AjustR2fixrand=AjustR2fixrand2)


        if(assessIC)  listGLMsp <- list(list(glm1,MODconfint1,md2,MODconfint2)) else  listGLMsp <- list(list(glm1,md2))
 ...
 ##browser() 
  ...
  if(description) {
    ...
    p <- p + geom_hline(data =hline.data,mapping = aes(yintercept=z, colour = couleur,linetype=type ),
                        alpha=1,size=1.2)
   if(is.numeric(tab1t$UL)==TRUE){ ############# ONLY FOR THE CONFIDENCE INTERVAL
